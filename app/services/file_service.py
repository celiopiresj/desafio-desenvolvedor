...
async def get_distinct_filenames(filename: str, exact_match: bool):
    if exact_match:
        return await db["files"].distinct("Filename", {"Filename": filename})
    else:
        regex = Regex(f"^{filename}", "i")
        return await db["files"].distinct("Filename", {"Filename": {"$regex": regex}})


async def paginate_file_by_name(filename: str, page: int, page_size: int, exact_match: bool):

    names = await get_distinct_filenames(filename, exact_match)
    ...
    for name in names:
        pipeline = [
            ...
            {"$skip": (page - 1) * page_size},
            {"$limit": page_size},
            {"$project": {"_id": 0, "Upload_date": 0, "Filename": 0}}
        ]

        response = db["files"].aggregate(pipeline)
        result = await response.to_list(length=page_size)

        total_documents = await db["files"].count_documents({"Filename": name})
        total_pages = (total_documents + page_size - 1) // page_size

        files.append({"total_pages": total_pages,
                     "filename": name, "data": result})
...
async def filter_files_by_name(filename: str, exact_match: bool):
    ...
    for name in names:
        ...
        response = db["files"].aggregate(pipeline)
        result = await response.to_list()

        files.append({"filename": name, "data": result})
...
async def get_files_by_name(filename: str, page: int, page_size: int,  paginate: bool, exact_match: bool):
    try:
        if paginate:
